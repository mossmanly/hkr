...
-- Directly references source table (no staging layer needed)
...
fee_calculations AS (
    SELECT
        ...
        -- Using CASE statements with MAX to handle one row per property
        ...
        -- CONFIGURATION PERCENTAGES (mapped to original column names)
    ...
    GROUP BY p.property_id, p.property_name, p.company_id, p.portfolio_id, 
...
SELECT
    ...
    ROUND((COALESCE(property_mgmt_base_pct, 0) + COALESCE(asset_mgmt_base_pct, 0) + 
           COALESCE(leasing_base_pct, 0) + COALESCE(maintenance_base_pct, 0))::numeric, 1) AS total_base_pct_of_pgi,
    ...
    CONCAT('Professional fees: ', 
           ROUND((COALESCE(property_mgmt_base_pct, 0) + COALESCE(asset_mgmt_base_pct, 0) + 
                  COALESCE(leasing_base_pct, 0) + COALESCE(maintenance_base_pct, 0))::numeric, 1), 
           '% of PGI calculated from configuration') AS fee_basis_notes,
