...
hurdle_tiers AS (
  ...
  FROM {{ source('hkh_dev', 'tbl_hurdle_tiers') }}
),
...
--------------------------------------------------------------------------------
-- Final clean output without helper columns - ROUNDED TO NO DECIMALS
--------------------------------------------------------------------------------
SELECT
  portfolio_id,
  year,
  ...
  ROUND(hurdle1_investor_calc, 0) AS hurdle1_investor,
  ROUND(hurdle1_sponsor_calc, 0) AS hurdle1_sponsor,
  ROUND(hurdle2_investor_calc, 0) AS hurdle2_investor,
  ROUND(hurdle2_sponsor_calc, 0) AS hurdle2_sponsor,
  ROUND(hurdle3_investor_calc, 0) AS hurdle3_investor,
  ROUND(hurdle3_sponsor_calc, 0) AS hurdle3_sponsor,
  ROUND(residual_investor_calc, 0) AS residual_investor,
  ROUND(residual_sponsor_calc + target_irr_excess_to_sponsor, 0) AS residual_sponsor,
  
  -- Party totals (ALL distributions)
  ROUND(pref_roc_paid + pref_irr_paid + common_roc_paid + common_irr_paid + hurdle1_investor_calc + hurdle2_investor_calc + hurdle3_investor_calc + residual_investor_calc, 0) AS total_investor,
  ROUND(hurdle1_sponsor_calc + hurdle2_sponsor_calc + hurdle3_sponsor_calc + residual_sponsor_calc + target_irr_excess_to_sponsor, 0) AS total_sponsor,
